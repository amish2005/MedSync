...
app.post('/hos-login', (req, res) => {
    ...
    const query = 'SELECT * FROM hospital_details WHERE registration_no = ? AND password = ?';
...
app.get('/hos-dashboard', (req, res) => {
    ...
    const query = 'SELECT hospital_name FROM hospital_details WHERE registration_no = ?';
    db.query(query, [req.session.user.registration_no], (err, results) => {
        if (err) throw err;

        // Extract hospital_name and render the view
        const hospitalName = results.length > 0 ? results[0].hospital_name : 'Hospital Name Not Found';
